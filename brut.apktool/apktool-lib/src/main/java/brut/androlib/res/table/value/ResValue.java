...
public abstract class ResValue {
    ...
    // The data field holds an attribute resource identifier (referencing an attribute in the
    // current theme style, not a resource entry).
    public static final int TYPE_ATTRIBUTE = 0x02;
    // The string field holds string data. In addition, if data is non-zero then it is the string
    // block index of the string and assetCookie is the set of assets the string came from.
    ...
    // The data holds a dynamic res table reference, which needs to be resolved before it can be
    // used like TYPE_REFERENCE.
    public static final int TYPE_DYNAMIC_REFERENCE = 0x07;
    // The data an attribute resource identifier, which needs to be resolved before it can be
    // used like a TYPE_ATTRIBUTE.
    ...
    // Identifies the start of integer values that were specified as color constants (starting
    // with '#').
