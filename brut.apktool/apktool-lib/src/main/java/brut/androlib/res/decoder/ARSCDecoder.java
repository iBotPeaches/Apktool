...
public class ARSCDecoder {
	...
	public static ARSCData decode(InputStream arscStream, boolean findFlagsOffsets, boolean keepBroken,
		...
		try {
			...
			return new ARSCData(pkgs,
					decoder.mFlagsOffsets == null ? null : decoder.mFlagsOffsets.toArray(new FlagsOffset[0]), resTable);
	...
	private ARSCDecoder(InputStream arscStream, ResTable resTable, boolean storeFlagsOffsets, boolean keepBroken) {
		...
		// We need to explicitly cast to DataInput as otherwise the constructor
		// is ambiguous.
		// We choose DataInput instead of InputStream as ExtDataInput wraps an
		// InputStream in
		// a DataInputStream which is big-endian and ignores the little-endian
		// behavior.
	...
	private ResPackage readTablePackage() throws IOException, AndrolibException {
		...
		if (id == 0) {
			// This means we are dealing with a Library Package, we should just
			// temporarily
			// set the packageId to the next available id . This will be set at
			// runtime regardless, but
			// for Apktool's use we need a non-zero packageId.
			// AOSP indicates 0x02 is next, as 0x01 is system and 0x7F is
			// private.
			id = 2;
	...
	private void readTableTypeSpec() throws AndrolibException, IOException {
		...
		while (type == Header.TYPE_SPEC_TYPE) {
			...
			// We've detected sparse resources, lets record this so we can
			// rebuild in that same format (sparse/not)
		...
		while (type == Header.TYPE_TYPE) {
			...
			if (mCountIn.getCount() < mHeader.endPosition) {
				LOGGER.warning(
						"Unknown data detected. Skipping: " + (mHeader.endPosition - mCountIn.getCount()) + " byte(s)");
	...
	private ResType readTableType() throws IOException, AndrolibException {
		...
		// For some APKs there is a disconnect between the reported size of
		// Configs
	...
	private ResIntBasedValue readValue() throws IOException, AndrolibException {
		...
		return type == TypedValue.TYPE_STRING ? mPkg.getValueFactory().factory(mTableStrings.getHTML(data), data)
	...
	private ResConfigFlags readConfigFlags() throws IOException, AndrolibException {
		...
		return new ResConfigFlags(mcc, mnc, language, country, orientation, touchscreen, density, keyboard, navigation,
				...
				screenWidthDp, screenHeightDp, localeScript, localeVariant, screenLayout2, colorMode, isInvalid, size);
	}
	...
	private void addMissingResSpecs() throws AndrolibException {
		...
		for (int i = 0; i < mMissingResSpecs.length; i++) {
			...
			ResResSpec spec = new ResResSpec(new ResID(resId | i), "APKTOOL_DUMMY_" + Integer.toHexString(i), mPkg,
					mTypeSpec);
	...
	private void checkChunkType(int expectedType) throws AndrolibException {
		if (mHeader.type != expectedType) {
			throw new AndrolibException(
					String.format("Invalid chunk type: expected=0x%08x, got=0x%08x", expectedType, mHeader.type));
		}
	}
