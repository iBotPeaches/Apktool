...
public class AXmlResourceParser implements XmlResourceParser {
	...
	public void open(InputStream stream) {
		close();
		if (stream != null) {
			// We need to explicitly cast to DataInput as otherwise the
			// constructor is ambiguous.
			// We choose DataInput instead of InputStream as ExtDataInput wraps
			// an InputStream in
			// a DataInputStream which is big-endian and ignores the
	...
	public String getAttributeNamespace(int index) {
		...
		// Minifiers like removing the namespace, so we will default to default
		// namespace
		// unless the pkgId of the resource is private. We will grab the
		// non-standard one.
	...
	public String getAttributeValue(int index) {
		...
		if (mAttrDecoder != null) {
			try {
				attrValue = mAttrDecoder.decode(valueType, valueData,
			...
			attrValue = TypedValue.coerceToString(valueType, valueData);
		}
		return attrValue;
	}
	...
	private static final int ATTRIBUTE_IX_NAMESPACE_URI = 0, ATTRIBUTE_IX_NAME = 1, ATTRIBUTE_IX_VALUE_STRING = 2,
			ATTRIBUTE_IX_VALUE_TYPE = 3, ATTRIBUTE_IX_VALUE_DATA = 4, ATTRIBUTE_LENGTH = 5;

	private static final int CHUNK_AXML_FILE = 0x00080003, CHUNK_AXML_FILE_BROKEN = 0x00080001,
			CHUNK_RESOURCEIDS = 0x00080180, CHUNK_XML_FIRST = 0x00100100, CHUNK_XML_START_NAMESPACE = 0x00100100,
			CHUNK_XML_END_NAMESPACE = 0x00100101, CHUNK_XML_START_TAG = 0x00100102, CHUNK_XML_END_TAG = 0x00100103,
