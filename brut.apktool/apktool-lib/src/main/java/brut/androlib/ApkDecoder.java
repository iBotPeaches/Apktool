...
public class ApkDecoder {
	...
	public static void setApkFile(File apkFile) {
		if (mApkFile != null) {
			try {
				mApkFile.close();
			...
			if (hasResources()) {
				...
				case DECODE_RESOURCES_NONE:
					...
					if (mForceDecodeManifest == FORCE_DECODE_MANIFEST_FULL) {
						...
						// done after raw decoding of resources because
						// copyToDir overwrites dest files
						if (hasManifest()) {
				...
				// if there's no resources.arsc, decode the manifest without
				// looking
	...
	private void putPackageInfo(MetaInfo meta) throws AndrolibException {
		...
		// only put rename-manifest-package into apktool.yml, if the change will
		// be required
